...
function preexec_invoke_cmd () {
    # Renable these two options to catch subshells.
...
function preexec_install () {
    ...
    # in ( ) subshells.  The null stderr redirections are to quiet errors on
    # bash2.05 (i.e. OSX's default shell) where the options can't be set, and
    # it's impossible to inherit the trap into subshells.
    set -o functrace > /dev/null 2>&1
    shopt -s extdebug > /dev/null 2>&1
